...
cc -shared -Wl,-soname,midi-fluidsynth.so -o midi-fluidsynth.so -fpic -fvisibility=hidden -O2 -Wall midi-fluidsynth.c -I`pwd`/include -lfluidsynth -L`pwd`/lib/arm
cc -shared -Wl,-soname,midi-adlmidi.so -o midi-adlmidi.so -fpic -fvisibility=hidden -O2 -Wall $SPEEXDSP_COMPILE midi-adlmidi.c -I`pwd`/include -lADLMIDI $SPEEXDSP_LINK -L`pwd`/lib/arm
...
c++ -c -fpic -fvisibility=hidden -O2 -Wall -fno-exceptions emu_mt32.cpp -I$CURDIR/src/munt-2.8
cd munt-2.8/mt32emu
c++ -c -fpic -fvisibility=hidden -O3 -Wall -fno-exceptions -DMT32EMU_WITH_STD_SNPRINTF *.cpp
...
cd ../../../..
cc -shared -Wl,-soname,mt32-munt.so -o mt32-munt.so *.o src/*.o src/munt-2.8/mt32emu/*.o src/munt-2.8/mt32emu/sha1/*.o -lm $SPEEXDSP_LINK -lstdc++ -L../lib/arm
